...
if ($mode === 'many_accounts') {
    ...
    for ($i=1; $i <= $amount; $i++) {
        ...
        $uid = $database->register($userName, trz_password_hash($password), $email, $tribe, $act);
        if (!$uid) continue;
        ...
        if (!$beginnersProtection) {
            $protection = time();
            $database->query("UPDATE ".TB_PREFIX."users SET protect = '".$protection."' WHERE id = ".(int)$uid);
        }
